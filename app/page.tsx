...
const DEFAULT_COACH_IMAGE = "/oreo.png"; 
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    try {
      ...
      // Force update to new default Oreo image if old default was used
      const oldDefaultImage = "https://raw.githubusercontent.com/Tarikul-Islam-Anik/Animated-Fluent-Emojis/master/Emojis/Animals/Black%20Cat.png";
      if (savedCoachImage && savedCoachImage !== oldDefaultImage) {
        setCoachImage(savedCoachImage);
      } else {
        // Use new default and save it
        setCoachImage(DEFAULT_COACH_IMAGE);
        localStorage.setItem('snapcalorie_coach_image', DEFAULT_COACH_IMAGE);
      }
  ...
  const renderLaunchScreen = () => (
    <div className={`fixed inset-0 z-50 flex flex-col items-center justify-between overflow-hidden pb-safe ${theme === 'neon' ? 'bg-black text-neon-green font-retro' : 'bg-indigo-600 text-white'}`}>
        {/* Oreo Background Image */}
        <div 
          className="absolute inset-0 opacity-20"
          style={{
            backgroundImage: 'url(/oreo.png)',
  ...
  const renderDashboard = () => {
    ...
    <div className="pb-10">
      ...
      <div className="grid grid-cols-1 gap-6 mb-6">
        <CalorieCard consumed={dailyTotals.calories} goal={effectiveCalorieGoal} theme={theme} coachImage={coachImage} />
      ...
      <div className={`mb-6 p-5 rounded-3xl relative overflow-hidden ${t.card}`}>
          {/* Background Image with Oreo Playing */}
          <div 
            className="absolute inset-0 opacity-30 pointer-events-none"
            style={{
              backgroundImage: 'url(/oreo_play.png)',
              backgroundSize: 'cover',
              backgroundPosition: 'center'
            }}
          />
      ...
      <div className={`p-5 rounded-3xl mb-6 relative overflow-hidden ${t.card}`}>
        {/* Background Image with Oreo */}
        <div 
          className="absolute inset-0 opacity-30 pointer-events-none"
          style={{
            backgroundImage: 'url(/oreo_email.png)',
            backgroundSize: 'cover',
            backgroundPosition: 'center'
          }}
        />
